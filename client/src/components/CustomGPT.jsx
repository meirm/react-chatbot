...
const CustomGPT = ({setCustomGPT}) => {
    // return a dropdown menu with the system prompts
    return (
        <div className="customGPTWrapper">
            <div className="title">Custom GPTs</div>
            <div className="options">
            <select className="select_gpt" onChange={(e) => setCustomGPT(e.target.value)}>
                <option value="default">Select a System Prompt</option>
                {systemPrompts.map((prompt, idx) => {
                    return (
                        <option key={idx} value={prompt.label}>{prompt.label}</option>
                    );
                })}
            </select>
            </div>
