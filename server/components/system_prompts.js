...
// We will fetch the system prompts from the repo https://github.com/meirm/UniversalPrompts under repository/{label}/system.md
 ...
 const fetchSystemPrompt = async (label) => {
...
const getSystemPrompts = async() => {
        ...
        for (let i = 0; i < systemPrompts.length; i++){
            console.log("Reading system prompt for: ",systemPrompts[i].label);
            const sysPrompt = await fetchSystemPrompt(systemPrompts[i].label);
            prompts[systemPrompts[i].label] = {value: systemPrompts[i].value, prompt: sysPrompt};
        }
        return prompts;
    }
 ...
 class CustomGPT{
    ...
    async fetchSystemPrompt(){
        this.systemPrompt = await fetchSystemPrompt(this.label);
    }
    
    async getSystemPrompt(label){
        return await fetchSystemPrompt(label);
    }
    async getSystemPrompts(){
        const prompts = {};
        for (let i = 0; i < systemPrompts.length; i++){
            const prompt = await this.getSystemPrompt(systemPrompts[i].label);
            prompts[systemPrompts[i].label] = {value: systemPrompts[i].value, prompt: prompt};
        }
        return prompts;
    }
}

module.exports = {systemPrompts, fetchSystemPrompt, getSystemPrompts,  CustomGPT};
